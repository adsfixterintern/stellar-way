...
const MyOrdersPage = () => {
  ...
  return (
    <div className="w-full p-6">
                ...
                orders.map((order: any) => {
                  ...
                  return (
                    <tr key={order._id} className="hover:bg-gray-50/30 transition-colors">
                      ...
                      <td className="p-5">
                        <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${isDelivered ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-600"}`}>
                      ...
                      <td className="p-5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {!isDelivered && (
                            <button onClick={() => handleTrackClick(order)} className="inline-flex items-center gap-2 px-4 py-2 text-[10px] font-black uppercase bg-[#1A4E11] text-white rounded-lg">
                              <IoLocationOutline size={20} /> Track
                            </button>
                          )}
                          {isDelivered && (
                            <button onClick={() => openReviewModal(order)} className="inline-flex items-center gap-2 px-4 py-2 text-[10px] font-black uppercase bg-[#1A4E11] text-white rounded-lg w-full justify-center">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-md">
          <div className="bg-white w-full max-w-xl shadow-2xl rounded-3xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b flex justify-between items-center bg-gray-50/50">
              <div>
                <h2 className="text-lg font-black text-gray-900 uppercase tracking-tight">Feedback</h2>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Order #{selectedOrder.transactionId?.slice(-8)}</p>
              </div>
              <button onClick={() => setIsReviewModalOpen(false)}><IoCloseOutline size={28} /></button>
            </div>
            ...
            <div className="p-6 max-h-[65vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="mb-10">
                <div className="flex items-center gap-2 mb-6">
                  <div className="p-2 bg-green-100 text-[#1A4E11] rounded-lg"><IoFastFoodOutline size={18} /></div>
                  <h3 className="font-black text-xs uppercase tracking-widest text-gray-500">Rate Food Items</h3>
                </div>
                      ...
                      <div key={mId || idx} className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                        <div className="flex items-center gap-4 mb-4">
                          <div className="w-12 h-12 bg-white rounded-xl relative overflow-hidden shrink-0 shadow-sm">
                            <Image src={menu?.image?.url || "/placeholder.png"} alt="food" fill className="object-cover" />
                          </div>
                          <div>
                            <p className="font-black text-sm text-gray-900">{menu?.title || "Food Item"}</p>
                            <p className="text-[9px] text-gray-400 font-black uppercase">Quantity: {item.quantity}</p>
                          </div>
                        </div>
                        <div className="flex gap-1.5 mb-3">
                          {[1, 2, 3, 4, 5].map((star) => (
                            <button key={star} onClick={() => handleRatingChange(mId, star)} className={`transition-all ${reviewData[mId]?.rating >= star ? "text-yellow-400 scale-110" : "text-gray-200"}`}><IoStar size={24} /></button>
                          ))}
                        </div>
                        <textarea placeholder="How was the taste?" value={reviewData[mId]?.comment || ""} onChange={(e) => handleCommentChange(mId, e.target.value)} className="w-full p-3 bg-white border border-gray-200 rounded-xl text-xs outline-none focus:border-[#1A4E11] resize-none" rows={2} />
                      </div>
                    );
                  })}
                </div>
              ...
              {selectedOrder?.riderId && (
                <div className="pt-8 border-t border-dashed border-gray-200 mb-10">
                  <div className="flex items-center gap-2 mb-6">
                    <div className="p-2 bg-orange-100 text-orange-600 rounded-lg"><IoBicycleOutline size={18} /></div>
                  ...
                  <div className="bg-orange-50/30 p-6 rounded-2xl border border-orange-100 flex flex-col items-center text-center">
                    ...
                    <h4 className="font-black text-gray-900 text-sm">{selectedOrder.riderId.userId?.name || "Delivery Hero"}</h4>
                    <div className="flex gap-2 justify-center my-4">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button key={star} onClick={() => setRiderRating(star)} className={`transition-all ${riderRating >= star ? "text-orange-500 scale-110" : "text-gray-200"}`}><IoStar size={28} /></button>
                      ))}
                    </div>
                    <textarea placeholder="Any comments for the rider?" value={riderComment} onChange={(e) => setRiderComment(e.target.value)} className="w-full p-3 bg-white border border-orange-100 rounded-xl text-xs outline-none resize-none" rows={2} />
                  </div>
                </div>
              )}
              ...
              <div className="pt-8 border-t border-dashed border-gray-200">
                <div className="flex items-center gap-2 mb-6">
                  <div className="p-2 bg-blue-100 text-blue-600 rounded-lg"><IoRestaurantOutline size={18} /></div>
                  <h3 className="font-black text-xs uppercase tracking-widest text-gray-500">Share on our Wall</h3>
                </div>
                <div className="bg-blue-50/30 p-6 rounded-2xl border border-blue-100">
                  <p className="text-xs font-bold text-gray-700 mb-3 uppercase tracking-tighter">Would you like to be featured on our home page?</p>
                  <textarea
            ...
            <div className="p-6 border-t bg-gray-50/50">
              <button onClick={handleSubmitReviews} disabled={btnLoading} className="w-full py-4 bg-[#1A4E11] text-white font-black rounded-2xl uppercase text-xs tracking-widest shadow-xl disabled:bg-gray-300 transition-all">
      ...
      {/* Track Modal (Keeping existing logic) */}
