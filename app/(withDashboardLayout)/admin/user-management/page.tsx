...
const UserManagementPage = () => {
  ...
  // স্ট্যাটাস (Block/Active) হ্যান্ডলার
  ...
  const handleRoleChange = async (id: string, newRole: string) => {
    try {
      const { data } = await api.patch(`/auth/users/update-role/${id}`, { role: newRole });
      if (data.success) {
        toast.success("Role updated successfully");
        fetchUsers(meta.page);
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (data.success) {
          toast.success("User deleted successfully");
          fetchUsers(users.length === 1 && meta.page > 1 ? meta.page - 1 : meta.page);
        }
      } catch (err: any) {
        toast.error(err.response?.data?.message || "Delete failed");
      }
  ...
  return (
    <div className="w-full p-2 md:p-4">
      {/* HEADER & SEARCH UI (Same as before) */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
        <div>
          <h1 className="text-xl md:text-2xl font-black text-gray-900 tracking-tight uppercase">User Management</h1>
          <p className="text-[10px] text-[#1A4E11] font-black uppercase tracking-widest mt-1">Total Records: {meta.total}</p>
        </div>
        <div className="flex flex-col sm:flex-row w-full lg:w-auto gap-3">
          <div className="relative grow sm:w-64">
            <IoSearchOutline className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
            <input
              ...
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-100 rounded-xl text-[11px] font-bold outline-none focus:border-[#1A4E11] transition-all"
            />
          </div>
          <div className="relative sm:w-44">
            <IoFilterOutline className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
            <select
              value={roleFilter}
              onChange={(e) => setRoleFilter(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-100 rounded-xl text-[11px] font-black uppercase tracking-wider outline-none focus:border-[#1A4E11] shadow-sm appearance-none"
            >
              <option value="">All Roles</option>
              <option value="user">User</option>
              <option value="admin">Admin</option>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                <th className="p-5 text-[10px] font-black uppercase text-gray-400 tracking-widest">User Info</th>
                <th className="p-5 text-[10px] font-black uppercase text-gray-400 tracking-widest text-center">Role</th>
                <th className="p-5 text-[10px] font-black uppercase text-gray-400 tracking-widest text-center">Account Status</th>
                <th className="p-5 text-[10px] font-black uppercase text-gray-400 tracking-widest text-right">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {loading ? <TableSkeleton /> : users.map((user) => (
                <tr key={user._id} className="hover:bg-gray-50/30 transition-colors">
                  ...
                  {/* স্ট্যাটাস ড্রপডাউন (Block Option) */}
                  <td className="p-5 text-center">
                    <select
                      value={user.status}
                      onChange={(e) => handleStatusChange(user._id, e.target.value)}
                      className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider outline-none border cursor-pointer transition-all ${
                        user.status === "active"
                          ? "bg-green-50 text-[#1A4E11] border-green-100 focus:border-[#1A4E11]"
                          : "bg-red-50 text-red-500 border-red-100 focus:border-red-500"
                      }`}
                    >
