...
import { Trash2, Plus, Minus, X, ShoppingBag, Clock, Eye, ShoppingCart } from "lucide-react";
...
const CartPage = () => {
  ...
  const handleProceedToCheckout = () => {
    ...
    localStorage.removeItem("pending_order_id"); // নতুন অর্ডার
  ...
  const handleExistingOrderPayment = (order: any) => {
    ...
    localStorage.setItem("pending_order_id", order._id); // এক্সিসটিং অর্ডার আইডি
      ...
      <div className="max-w-7xl mx-auto px-4 mt-16">
        ...
        {unpaidOrders.length > 0 && (
          <div className="mb-12 p-8 bg-[#f8faf7] rounded-[40px] border border-[#e8f0e5] shadow-sm">
            <h3 className="text-sm font-bold text-[#3D5334] uppercase tracking-widest mb-6 flex items-center gap-2">
              <Clock size={18} className="text-amber-500" /> Pending Unpaid Orders
            </h3>
              ...
              {unpaidOrders.map((order) => (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <Eye size={16} className="text-gray-300 group-hover:text-[#3D5334]" />
                  </div>
                  <p className="text-2xl font-black text-[#1D3A15]">৳{order.totalPrice.toFixed(2)}</p>
        ...
        {cartItems.length === 0 ? (
          <div className="flex flex-col items-center py-20 text-center">
            <ShoppingBag size={80} strokeWidth={1} className="text-gray-100 mb-6" />
            <h3 className="text-2xl font-bold text-gray-400">Your cart is empty</h3>
            <Link href="/" className="mt-4 text-[#3D5334] font-bold hover:underline">Go back to shop</Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2 space-y-6">
              {cartItems.map((item) => (
                <div key={item._id} className={`relative bg-white border ${selectedItems.includes(item._id) ? "border-[#1D3A15]" : "border-gray-100"} rounded-[30px] p-5 flex gap-6 items-center shadow-sm transition-all`}>
                  <input type="checkbox" className="w-5 h-5 accent-[#1D3A15] cursor-pointer" checked={selectedItems.includes(item._id)} onChange={() => toggleSelectItem(item._id)} />
                  <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-2xl overflow-hidden bg-gray-50 shrink-0">
                    <Image src={item.image?.url || item.image || "/placeholder.jpg"} alt="Food" fill className="object-cover" />
                  </div>
                  <div className="grow">
                    <div className="flex justify-between items-start">
                      <h3 className="text-2xl font-bold text-gray-800">{item.name || item.title}</h3>
                      <button onClick={() => removeItem(item._id)} className="text-gray-300 hover:text-red-500 transition-colors"><X size={20} /></button>
                    </div>
                    ...
                    <div className="flex items-center border border-gray-200 w-fit rounded-xl px-2 py-1 gap-5 mt-4">
                      <button onClick={() => handleQuantityChange(item._id, -1)} className="p-1 text-gray-400 hover:text-black"><Minus size={16} /></button>
                      <span className="font-bold text-sm min-w-[20px] text-center">{item.quantity}</span>
                      <button onClick={() => handleQuantityChange(item._id, 1)} className="p-1 text-gray-400 hover:text-black"><Plus size={16} /></button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="lg:col-span-1">
              <div className="bg-white border border-gray-100 shadow-sm rounded-[35px] p-8 sticky top-10">
                ...
                <div className="flex justify-between items-center mb-8">
                  <span className="text-gray-600 font-bold text-lg">Grand Total</span>
                  <span className="text-2xl font-black text-[#1D3A15]">৳{grandTotal.toFixed(2)}</span>
                </div>
                <button onClick={handleProceedToCheckout} className="w-full bg-[#1D3A15] text-white py-5 rounded-2xl font-bold uppercase tracking-widest hover:bg-black transition-all shadow-xl">
                  Proceed to Checkout
                </button>
              </div>
            </div>
      ...
      {selectedOrderDetails && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-[45px] p-10 relative">
            <button onClick={() => setSelectedOrderDetails(null)} className="absolute right-8 top-8 p-2 bg-gray-50 rounded-full hover:text-red-500 transition-all"><X size={20} /></button>
            <h3 className="text-2xl font-bold text-[#1D3A15] mb-8">Order Breakdown</h3>
            <div className="space-y-4 max-h-[350px] overflow-y-auto pr-3 mb-8">
              {selectedOrderDetails.items.map((item: any, idx: number) => (
                <div key={idx} className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl">
                  <span className="font-bold text-gray-800 text-sm">{item.menuId?.name || item.menuId?.title || "Food Item"} (x{item.quantity})</span>
                  <span className="font-bold text-[#1D3A15]">৳{(item.quantity * item.price).toFixed(2)}</span>
                </div>
              ))}
            </div>
            <button onClick={() => handleExistingOrderPayment(selectedOrderDetails)} className="w-full bg-[#3D5334] text-white py-4 rounded-2xl font-bold hover:bg-[#1D3A15] transition-all">
