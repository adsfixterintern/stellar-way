...
export const MenuModal = ({ isOpen, onClose, editData, categories, chefs, onSuccess }: MenuModalProps) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const url = editData ? `${BASE_URL}/menu/${editData._id}` : `${BASE_URL}/menu/create-menu`;
      const response = await axios({ method: editData ? "patch" : "post", url, data });
    ...
    <div className="fixed inset-0 z-10 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-2xl shadow-2xl rounded-2xl overflow-hidden">
        <div className="flex justify-between items-center p-6 border-b border-gray-50">
          <h2 className="text-xl font-black text-gray-900">{editData ? "Edit Menu Item" : "Add New Menu"}</h2>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full"><IoCloseOutline size={24} /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-4 max-h-[80vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <textarea required value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} rows={2} className="w-full border border-gray-100 p-4 rounded-xl outline-none focus:border-[#1A4E11] text-sm bg-gray-50 font-semibold resize-none" />
            </div>
            ...
            <div><label className="text-[10px] font-black uppercase text-gray-400 mb-2 block">Price</label><input type="number" required value={formData.price} onChange={(e) => setFormData({...formData, price: e.target.value})} className="w-full border border-gray-100 p-4 rounded-xl outline-none text-sm bg-gray-50 font-semibold" /></div>
            <div><label className="text-[10px] font-black uppercase text-gray-400 mb-2 block">Stock</label><input type="number" required value={formData.stock} onChange={(e) => setFormData({...formData, stock: e.target.value})} className="w-full border border-gray-100 p-4 rounded-xl outline-none text-sm bg-gray-50 font-semibold" /></div>
            ...
            <div className="md:col-span-2">
              <label className="relative flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-100 rounded-xl cursor-pointer hover:bg-gray-50 overflow-hidden">
                {imagePreview ? <Image src={imagePreview} alt="Preview" fill className="object-cover" /> : <div className="flex flex-col items-center"><IoImageOutline size={30} className="text-gray-200" /><span className="text-[9px] text-gray-400 mt-2 font-black uppercase">Upload Image</span></div>}
                <input type="file" className="hidden" accept="image/*" onChange={handleImageChange} />
              </label>
            </div>
          </div>
          <button type="submit" disabled={btnLoading} className="w-full bg-[#1A4E11] text-white py-4 rounded-xl font-black text-[11px] uppercase tracking-[2px]">{btnLoading ? "Processing..." : editData ? "Update" : "Save"}</button>
        </form>
      </div>
    </div>
  );
