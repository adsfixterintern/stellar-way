  ...
  const validate = () => {
    ...
    if (!isLogin) {
      ...
      if (!formData.phone) newErrors.phone = "Phone number is required";
        ...
        newErrors.phone = "Invalid phone (11 digits required)";
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8FAF8] p-4 md:p-10">
      <div className="relative bg-white w-full max-w-275 min-h-150 md:h-175 shadow-[0px_25px_80px_rgba(26,78,17,0.15)] rounded-[30px] md:rounded-[40px] overflow-hidden flex flex-col md:flex-row">
        ...
        <div
          className={`w-full md:w-1/2 flex items-center justify-center p-8 md:p-16 transition-all duration-500 ${!isLogin ? "hidden md:flex opacity-0" : "flex opacity-100"}`}
        >
          <div className="w-full max-w-100 flex flex-col">
            ...
            <div className="md:hidden mt-10 pt-6 border-t border-gray-100 flex flex-col items-center gap-4">
              <p className="description text-[14px]! normal-case">
                Do not have an account?
              </p>
              <button
                type="button"
                onClick={() => setIsLogin(false)}
                className="blockBtn w-full! h-12! bg-white! text-[#1a4e11]! border-2 border-[#1a4e11] rounded-xl! font-bold"
              >
                CREATE AN ACCOUNT
              </button>
            </div>
